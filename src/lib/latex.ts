...
import { type ConfidenceLevel } from './tTable';
...
export function generateLatex(data: ExportData): string {
  ...
  data.variableResults.forEach(vr => {
    ...
    if (n > 1) {
      ...
      lines.push(`u_A(${vr.name}) = t_p \\cdot \\frac{S_{${vr.name}}}{\\sqrt{n}} = ${formatNumber(tFactor)} \\times \\frac{${formatNumber(vr.stats.sampleStd)}}{\\sqrt{${n}}} = ${formatNumber(vr.stats.uA)}`);
      lines.push('\\end{equation}');
    }
    ...
    lines.push('\\begin{equation}');
    lines.push(`u_B(${vr.name}) = \\frac{\\Delta_{ins}}{k} = \\frac{${formatNumber(instError)}}{${formatNumber(distFactor)}} = ${formatNumber(vr.stats.uB)}`);
    lines.push('\\end{equation}');
    
    lines.push('\\textbf{合成不确定度:}');
    lines.push('\\begin{equation}');
    lines.push(`u_c(${vr.name}) = \\sqrt{u_A^2(${vr.name}) + u_B^2(${vr.name})} = ${formatNumber(vr.stats.uc)}`);
    lines.push('\\end{equation}');
    lines.push('');
  });
  ...
  lines.push(`\\textbf{相对不确定度:} $\\frac{u_c(${data.resultVariable})}{|${data.resultVariable}|} = ${formatNumber(data.relativeUncertainty * 100, 2)}\\%$`);
  lines.push('');
  lines.push('\\begin{equation}');
  lines.push(`\\boxed{${data.resultVariable} = ${data.formattedValue} \\pm ${data.formattedUncertainty} \\quad (P = ${data.confidence})}`);
  lines.push('\\end{equation}');
...
export function generateMarkdown(data: ExportData): string {
  ...
  data.variableResults.forEach(vr => {
    ...
    if (n > 1) {
      lines.push('**样本标准差:**');
      lines.push('$$');
      lines.push(`S_{${vr.name}} = \\sqrt{\\frac{\\sum(${vr.name}_i - \\bar{${vr.name}})^2}{${n}-1}} = ${formatNumber(vr.stats.sampleStd)}`);
      lines.push('$$');
      lines.push('');
  ...
  lines.push('## 6. 最终结果');
  lines.push('');
  lines.push(`**相对不确定度:** $\\frac{u_c(${data.resultVariable})}{|${data.resultVariable}|} = ${formatNumber(data.relativeUncertainty * 100, 2)}\\%$`);
  lines.push('');
  lines.push('$$');
  lines.push(`\\boxed{${data.resultVariable} = ${data.formattedValue} \\pm ${data.formattedUncertainty} \\quad (P = ${data.confidence})}`);
  lines.push('$$');
